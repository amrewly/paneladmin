...
$query = mysqli_query($con, "SELECT produk.*, kategori.nama AS nama_kategori FROM produk JOIN kategori ON produk.kategori_id = kategori.id WHERE produk.id = '$id'");
...
if (isset($_POST['simpan'])) {
    ...
    if ($nama == '' || $kategori == '' || $harga == '') {
        $pesan = "Nama, kategori, dan harga wajib diisi!";
    } else {
        ...
        if ($queryUpdate) {
            ...
            if ($nama_file != '') {
                if ($image_size > 500000) {
                    $pesan = "Ukuran file tidak boleh lebih dari 500 kb!";
                } else {
                    if ($imageFileType != 'jpg' && $imageFileType != 'png' && $imageFileType != 'gif') {
                        $pesan = "File harus bertipe JPG, PNG, atau GIF";
                    } else {
                        move_uploaded_file($_FILES["foto"]["tmp_name"], $target_dir . $new_name);
                        $queryUpdate = mysqli_query($con, "UPDATE produk SET foto='$new_name' WHERE id='$id'");
                        if (!$queryUpdate) {
                            $pesan = "Gagal mengupdate foto produk: " . mysqli_error($con);
                        }
                    }
                }
            }
        } else {
            $pesan = "Gagal mengupdate produk: " . mysqli_error($con);
        }
    }
}
...
if (isset($_POST['delete'])) {
    ...
    if ($queryDelete) {
        ...
        $pesan = "Gagal menghapus produk: " . mysqli_error($con);
    }
}
...
<body>
        ...
        <div class="col-12 col-md-6 mb-5">
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <textarea name="detail" id="" cols="30" rows="10" class="form-control"><?php echo $data['detail']?></textarea>
                </div>
                <div>
    ...
    <script src="bootstrap/jsbootstrap.bundle.min.js"></script>
</body>
</html>
